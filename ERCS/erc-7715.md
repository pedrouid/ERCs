...
`account` is an optional field that identifies the account being targetted for this permissions request.  It may be skipped only if a connection has not yet been established and this request serves as the connection request in addition to a permissions request, in which case the response will indicate the account.
...
type GrantPermissionsResponse = {
  ...
  signerData: {
    7679UserOpBuilder?: `0x${string}`;
    7710DelegationManagerAddress?: `0x${string}`;
  };
...
- `account` optionally indicates the account if an account was not specified in the request, and therefore the request serves also as a connection request.
...
- `signerData` includes data that the DApp will need in order to construct transactions for the account.  Currently it includes either a `7679UserOpBuilder` field for ERC-7679, or a `7710DelegationManager` field for ERC-7710.
...
`wallet_grantPermissions` replies with `permissionsContext` and `7679UserOpBuilder` address inside the `signerData` field. DApps can use that data with methods provided by [ERC-7679](./eip-7679.md) to build the [ERC-4337](./eip-4337.md) userOp.
...
When requesting permissions with a `type` of `account`, the returned data will be redeemable using the interfaces specified in ERC-7710. This allows the recipient of the permissions to use any account type (EOA or contract) to form a transaction or UserOp using whatever payment or relay infrastructure they prefer, by sending an internal message to the returned `permissions.signerData.7710DelegationManagerAddress` and calling its `function redeemDelegation(bytes calldata _data, Action calldata _action) external;` function with the `_data` parameter set to the returned `permissions.permissionsContext`, and the `_action` data forming the message that the permissions recipient desires the user's account to emit, as defined by this struct:
...
const managerAddress = permissionsResponse.signerData.7710DelegationManagerAddress;
